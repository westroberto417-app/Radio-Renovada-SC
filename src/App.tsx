...
// Memoized tab view router to prevent re-instantiating tabs unless activeTab changes
const TabContent = memo(({ activeTab }: { activeTab: Tab }) => {
  switch (activeTab) {
...
// Memoized static background visual layer
...
const App = () => {
  // Fine-grained atomic Zustand selectors to prevent re-renders when other state changes
  ...
  useEffect(() => {
    ...
    fetchRequestCount();
    const interval = setInterval(fetchRequestCount, 20000); // Poll every 20 seconds
